...
import { forceSuccess, downloadLog } from '@/service/modules/task-instances'
import { queryTaskListPaging } from '@/service/modules/log/task-log'
...
export function useTable() {
  ...
  const createColumns = (variables: any) => {
    variables.columns = [
      ...
      {
        title: t('security.log.project_name'),
        key: 'projectName',
        ...COLUMN_WIDTH_CONFIG['name']
      },
      {
      ...
      getTableData({
        ...
        pageNum:
          variables.tableData.length === 1 && variables.page > 1
            ? variables.page - 1
            : variables.page,
        taskInstanceName: variables.searchVal,
  ...
  const getTableData = (params: any) => {
    ...
    const data = {
      ...
      taskInstanceName: params.searchVal,
    ...
    const { state } = useAsyncState(
      queryTaskListPaging(data).then((res: TaskInstancesRes) => {
